...
async function loadSettings() {
  try {
    ...
    const photoUrlVal = settings.photo_url || '';
    updatePhotoPreview(photoUrlVal);
...
function updatePhotoPreview(url) {
  const preview = document.getElementById('setting-photo-preview');
  const placeholder = document.getElementById('setting-photo-placeholder');
  if (!preview) return;
  if (url && url.trim()) {
    preview.src = url;
...
async function handlePhotoUpload(input) {
  ...
  try {
    ...
    if (res.ok && data.photoUrl) {
      document.getElementById('setting-photo_url').value = data.photoUrl;
